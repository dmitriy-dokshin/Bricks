...
namespace Bricks.Auth.Providers
{
	public sealed class OAuthTokenProvider : OAuthBearerAuthenticationProvider
	{
		...
		{
			Func<IOwinRequest, string> getter = _locations.FirstOrDefault(x => !String.IsNullOrWhiteSpace(x(context.Request)));
			if (getter != null)
			{
				string tokenStr = getter(context.Request);
